...
public class BarCharDrawerForStudentsSem extends JFrame{
    public BarCharDrawerForStudentsSem(String title, ArrayList<Studentinfo> students,int min){
        super(title);
    ...
    public static JPanel creataStudentsPointForActPanel(ArrayList<Studentinfo> students,int min){
        JFreeChart chart=createBarChart(ChartDataMapper.creadetStudentsSemPoints(students,min));
        chart.setPadding(new RectangleInsets(4,8,2,2));
        return new ChartPanel(chart);
    }
    public  static JFreeChart createBarChart(CategoryDataset dataset){
        JFreeChart chart= ChartFactory.createBarChart(
                "Point Students for sem",
                "Student Name",
                "Sem Point",
                dataset,
